...
function evaluateFormulaLocal(formula, getData) {
    try {
...
function setupFormulaSupport(formulaInput, cellGrid, autocompleteEl, formulaListEl, getData, setCurrentCellFormula) {
...
function showAutocomplete(filter, formulaInput) {
    ...
    const filtered = FORMULA_LIST_LOCAL.filter(name => name.startsWith(filter)).slice(0, 10);
    if (filtered.length === 0) {
        hideAutocomplete();
        return;
    }
    filtered.forEach((name, index) => {
        const formula = FORMULAS_LOCAL[name];
 ...
 */
function calculateCellFormula(formula, currentRow, currentCol, getData) {
    if (!formula.startsWith('=')) {
        return formula;
    }
