...
function registerIPCHandlers() {
    electron_1.ipcMain.handle('ai-chat', async (event, { message, tableContext, mode, history = [] }) => {
        ...
        for (let i = 0; i < OPENROUTER_KEYS.length; i++) {
            ...
            try {
                const response = await new Promise((resolve, reject) => {
                    const req = https_1.default.request('https://openrouter.ai/api/v1/chat/completions', {
                        method: 'POST',
                        headers: {
    ...
    const template = [
        {
            ...
            submenu: [
                ...
                { type: 'separator' },
                { label: 'Выход', accelerator: process.platform === 'darwin' ? 'Cmd+Q' : 'Alt+F4', click: () => electron_1.app.quit() },
            ],
        },
        {
            label: 'Правка',
