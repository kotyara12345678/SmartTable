...
export type ChartType = 'bar' | 'horizontalBar' | 'line' | 'pie' | 'doughnut' | 'area' | 'radar' | 'scatter' | 'bubble' | 'polarArea';
...
export class ChartsWidget {
  ...
  createChartFromRange(data: ChartData, type: ChartType = 'bar', title: string = 'Диаграмма'): void {
  ...
  private render(): void {
    if (!this.chartConfig || !this.canvas) return;
    ...
    switch (type) {
      case 'bar': 
      case 'horizontalBar':
        this.drawBarChart(data, chartArea); break;
      case 'line': 
      case 'radar':
      case 'scatter':
        this.drawLineChart(data, chartArea); break;
      case 'pie': 
      case 'doughnut':
      case 'polarArea':
        this.drawPieChart(data, chartArea); break;
      case 'area': 
      case 'bubble':
        this.drawAreaChart(data, chartArea); break;
    }
    this.updateChartInfo();
  }

  private drawBarChart(data: ChartData, area: any): void {
    const ctx = this.ctx;
    if (!ctx) return;
  ...
  private drawLineChart(data: ChartData, area: any): void {
    const ctx = this.ctx;
    if (!ctx) return;
    const width = area.right - area.left;
    const height = area.bottom - area.top;
  ...
  private drawPieChart(data: ChartData, area: any): void {
    const ctx = this.ctx;
    if (!ctx) return;
  ...
  private drawAreaChart(data: ChartData, area: any): void {
    const ctx = this.ctx;
    if (!ctx) return;
    const width = area.right - area.left;
    const height = area.bottom - area.top;
    ...
    data.datasets.forEach((dataset, datasetIndex) => {
      ...
      dataset.data.forEach((value, index) => {
        const x = area.left + pointSpacing * index;
        const y = area.bottom - ((value - minValue) / range) * height;
        if (index === 0) { ctx.moveTo(x, area.bottom); ctx.lineTo(x, y); } 
        else { ctx.lineTo(x, y); }
      });
  ...
  private drawLegend(data: ChartData, x: number, y: number): void {
    const ctx = this.ctx;
    if (!ctx) return;
    const defaultColors = ['#4CAF50', '#2196F3', '#FFC107', '#F44336', '#9C27B0'];
  ...
  changeChartType(): void {
    ...
    const types: ChartType[] = ['bar', 'horizontalBar', 'line', 'pie', 'doughnut', 'area', 'radar', 'scatter', 'bubble', 'polarArea'];
  ...
  destroy(): void {
    if (this.element) { this.element.innerHTML = ''; this.element.remove(); }
    this.element = null;
    this.canvas = null;
    this.ctx = null;
  }
