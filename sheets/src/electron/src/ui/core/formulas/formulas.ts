 ...
 * Вычислить формулу
 */
export function evaluateFormula(formula: string, getData: (cell: string) => string): FormulaResult {
  try {
 ...
 */
function evaluateCondition(value: any): boolean {
 ...
 */
function parseIfArgs(argsStr: string, getData: (cell: string) => string): any[] {
  const args: any[] = [];
  let current = '';
  let parenDepth = 0;
  let inQuotes = false;
 ...
 */
function parseIfArg(arg: string, getData: (cell: string) => string): any {
 ...
 */
function findOperator(str: string, op: string): number {
  let inQuotes = false;
 ...
 */
function parseLogicalArgs(argsStr: string, getData: (cell: string) => string): any[] {
  const args: any[] = [];
 ...
 */
function parseSingleValue(value: string, getData: (cell: string) => string): any {
 ...
 */
function evaluateIfs(argsStr: string, getData: (cell: string) => string): FormulaResult {
 ...
 */
function parseArgs(argsStr: string, getData: (cell: string) => string): any[] {
  const args: any[] = [];
  const parts = splitArgs(argsStr);
 ...
 */
function getRangeValues(startCol: string, startRow: number, endCol: string, endRow: number, getData: (cell: string) => string): any[] {
  const values: any[] = [];
...
function formatNumber(value: number, format: string): string {
  ...
  if (format === '#,##0.00') return value.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  if (format === '#,##0') return Math.round(value).toLocaleString('ru-RU');
 ...
 */
function vlookup(args: any[], getData: (cell: string) => string): any {
 ...
 */
function hlookup(args: any[], getData: (cell: string) => string): any {
 ...
 */
function index(args: any[], getData: (cell: string) => string): any {
 ...
 */
function matchFunc(args: any[], getData: (cell: string) => string): any {
