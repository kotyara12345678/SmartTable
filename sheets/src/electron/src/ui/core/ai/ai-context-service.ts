...
import { databaseManager, ChatMessage, ChatSession } from '../../../database/simple-database.js';
...
export class AIContextService {
   ...
   */
  private async generateAIResponse(message: string, context: AIContext): Promise<AIResponse> {
   ...
   */
  async searchHistory(query: string, sessionId?: string): Promise<ChatMessage[]> {
    try {
   ...
   */
  async saveDocumentContext(documentId: string, content: string, metadata?: any): Promise<void> {
    try {
   ...
   */
  async getDocumentContext(documentId: string): Promise<any | null> {
    try {
  ...
  async getUsageStats(): Promise<any> {
    try {
      ...
      const recentActivity = await databaseManager.get(
        'SELECT COUNT(*) as count FROM chat_messages WHERE timestamp > datetime("now", "-24 hours")'
      );
  ...
  async exportChatHistory(sessionId: string): Promise<string> {
    try {
      const messages = await databaseManager.getMessages(sessionId);
      const session = await databaseManager.get(
