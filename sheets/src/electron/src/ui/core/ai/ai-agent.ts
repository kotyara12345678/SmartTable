...
export async function chatWithAI(message: string, tableContext?: string, currentMode?: AIMode): Promise<AIResponse> {
...
function parseCommands(content: string): TableCommand[] {
...
function parseModeSwitch(content: string): AIMode | undefined {
  const jsonRegex = /```json\s*([\s\S]*?)\s*```/g;
  let match;
  ...
  while ((match = jsonRegex.exec(content)) !== null) {
    try {
      ...
      if (parsed.suggestModeSwitch) {
        return parsed.suggestModeSwitch as AIMode;
      }
    } catch (e) {
      // Ignore parse errors
    }
...
export function executeCommand(command: TableCommand, callbacks: {
...
export function getTableContext(data: Map<string, { value: string; style?: any }>, rows: number, cols: number): string {
  const lines: string[] = [];
