...
const state = {
  ...
  cellFormulas: new Map<string, string>(), // ячейка -> формула
  // Фильтры и сортировка
  activeFilters: new Map<number, { values: string[]; type: 'include' | 'exclude' }>(), // колонка -> фильтр
  activeSort: { column: null as number | null, direction: null as 'asc' | 'desc' | null },
  // Группировка
  groupedColumns: new Set<number>(),
  // Поиск
  searchResults: new Set<string>(), // ячейки которые найдены
...
let globalCellInput: HTMLInputElement | null = null;

function getGlobalCellInput(): HTMLInputElement {
  if (!globalCellInput) {
    globalCellInput = document.getElementById('global-cell-input') as HTMLInputElement;
  }
...
let elements: {
  ...
  formulaSuggestions: HTMLElement;
  formulaSuggestionsList: HTMLElement;
...
function editCell(row: number, col: number, selectAll: boolean = true): void {
  ...
  if ((input as any)._currentInputHandler) {
    input.removeEventListener('input', (input as any)._currentInputHandler);
  }
  ...
  const handleFormulaInput = () => {
    ...
    if (currentValue.startsWith('=')) {
      const result = previewFormula(currentValue, (cellRef: string) => {
        const match = cellRef.match(/^([A-Z]+)(\d+)$/i);
        if (!match) return '';
  ...
  (input as any)._currentInputHandler = handleFormulaInput;
...
function finishEditing(save: boolean = true): void {
  if (state.editingCell.row === -1) return; // Нет редактируемой ячейки
...
function recalculateDependentCells(changedCellKey: string, data: Map<string, any>): void {
  ...
  for (const depKey of dependents) {
    ...
    const result = previewFormula(formula, (cellRef: string) => {
      const match = cellRef.match(/^([A-Z]+)(\d+)$/i);
      if (!match) return '';
      const refCol = letterToCol(match[1]);
      const refRow = parseInt(match[2]) - 1;
 ...
 */
function getHexColorByName(colorName: string): string | null {
 ...
 */
function applyCellColor(row: number, col: number, hexColor: string, data: Map<string, any>): void {
  const key = getCellKey(row, col);
...
function autoFitColumn(col: number): void {
  ...
  // Применить ширину
  const columnHeader = elements.columnHeaders.children[col] as HTMLElement;
  if (columnHeader) {
...
function setupSheetContextMenu(): void {
  ...
  document.addEventListener('click', (e: MouseEvent) => {
    const item = (e.target as HTMLElement).closest('.context-menu-item') as HTMLElement;
    if (!item || !item.closest('#sheetContextMenu')) return;
...
function handleSheetContextMenuAction(action: string): void {
  ...
  switch (action) {
    ...
    case 'move-sheet-right':
      {
        const index = state.sheets.findIndex(s => s.id === sheetId);
        if (index < 0 || index >= state.sheets.length - 1) return; // Уже последний
        const sheet = state.sheets[index];
        state.sheets.splice(index, 1);
        state.sheets.splice(index + 1, 0, sheet);
        renderSheets();
    ...
    case 'delete-sheet':
      {
        ...
        if (!confirm('Вы уверены, что хотите удалить этот лист?')) return;
...
function showFindInSelectionModal(): void {
  ...
  btnSearchNow.addEventListener('click', () => {
    ...
    const caseSensitive = (document.getElementById('searchCaseSensitive') as HTMLInputElement).checked;
    const exactMatch = (document.getElementById('searchExactMatch') as HTMLInputElement).checked;
...
function performSearch(query: string, caseSensitive: boolean, exactMatch: boolean, resultsContainer: HTMLElement): void {
  const data = getCurrentData();
  const results: Array<{ row: number; col: number; value: string; cellKey: string }> = [];
  ...
  if (results.length === 0) {
    ...
    document.addEventListener('goto-cell', ((e: any) => {
...
function showFilterByValueModal(columnIndex: number): void {
  ...
  btnApplyFilter.addEventListener('click', () => {
    const filterType = (document.getElementById('filterType') as HTMLSelectElement).value as 'include' | 'exclude';
...
function getCellValueForSort(cellData: any): number | string {
  if (!cellData || !cellData.value) return ''; // Пустые ячейки в конец
...
function setupEventListeners(): void {
  ...
  document.addEventListener('fill-color-change', (e: Event) => {
    const event = e as CustomEvent<{ color: string }>;
  ...
  document.addEventListener('open-template-manager', () => {
    const component = document.querySelector('template-manager-component') as any;
  ...
  elements.formulaInput.addEventListener('input', (e) => {
    ...
    if (cell) {
      ...
      if (value.startsWith('=')) {
        ...
        const result = previewFormula(value, (cellRef: string) => {
          const match = cellRef.match(/^([A-Z]+)(\d+)$/i);
          if (!match) return '';
          const refCol = letterToCol(match[1]);
          const refRow = parseInt(match[2]) - 1;
    ...
    if (value.startsWith('=')) {
      ...
      if (lastPart.length > 0) {
        showFormulaSuggestions(
          lastPart,
          elements.formulaInput,
          elements.formulaSuggestions,
          elements.formulaSuggestionsList
        );
      } else {
  ...
  elements.formulaInput.addEventListener('keydown', (e) => {
    ...
    const handled = handleFormulaSuggestionsKeydown(
      e,
      elements.formulaSuggestions,
      elements.formulaSuggestionsList,
      elements.formulaInput
  ...
  document.addEventListener('click', (e) => {
    if (!elements.formulaSuggestions.contains(e.target as Node) && e.target !== elements.formulaInput) {
      hideFormulaSuggestions(elements.formulaSuggestions);
    }
  });
...
function updateRowHeight(row: number, height: number): void {
  ...
  cells.forEach(cell => {
    ...
    (cell as HTMLElement).style.minHeight = `${height}px`;
  ...
  document.addEventListener('mouseup', (e) => {
    ...
    if (cell) {
      ...
      for (const target of targetCells) {
        ...
        if (finalValue) {
          const newCellData: { value: string; formula?: string; style?: any } = { value: finalValue };
