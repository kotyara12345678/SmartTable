...
export class SimpleDatabaseManager {
  ...
  private documentContexts: Map<string, any> = new Map();
  ...
  private loadFromStorage(): void {
    try {
      ...
      const maxId = Math.max(
        ...this.messages.map(m => m.id || 0),
        ...this.sessions.map(s => s.id || 0),
        ...this.preferences.map(p => p.id || 0),
        0
      );
      this.nextId = maxId + 1;
    } catch (error) {
  ...
  async cleanupOldData(daysOld: number = 30): Promise<void> {
    try {
      ...
      this.sessions = this.sessions.filter(s => 
        new Date(s.updated_at) >= cutoffDate && s.message_count > 0
      );
