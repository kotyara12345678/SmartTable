...
class SpreadsheetWidget(QTableWidget):
    ...
    def init_ui(self):
        ...
        # Скрываем стандартную Qt corner button и создаём свою
    ...
    def _hide_qt_corner_button(self):
        ...
        # Скрываем corner widget полностью
        ...
        # Находим и скрываем все corner buttons
    ...
    def _create_custom_corner_button(self, custom_width=100, custom_height=34):
        ...
        Args:
            custom_width: Кастомная ширина кнопки (int) или None для авто
            custom_height: Кастомная высота кнопки (int) или None для авто
        """
        from PyQt5.QtWidgets import QPushButton
        from PyQt5.QtCore import QTimer

        # Создаём собственную кнопку
        self._corner_button = QPushButton(self)
        self._corner_button.setObjectName("custom_corner_button")

        # Сохраняем кастомные размеры для последующего использования
        self._corner_custom_width = custom_width
        self._corner_custom_height = custom_height

        # Устанавливаем фиксированный размер сразу
        self._corner_button.setFixedSize(custom_width, custom_height)

        # Устанавливаем иконку или текст (можно использовать символ для выделения всех)
        ...
        # Применяем стили с задержкой, чтобы тема успела загрузиться
        QTimer.singleShot(150, lambda: self._apply_corner_button_theme(self._corner_button))

        # Размещаем кнопку в углу таблицы
        QTimer.singleShot(100, self._position_corner_button)

        # Обновляем позицию при изменении размеров заголовков
    ...
    def _apply_corner_button_theme(self, button):
        ...
        # Определяем текущую тему по цвету фона заголовков таблицы
        # (они уже правильно стилизованы темой)
        is_dark = False
        
        try:
            header = self.horizontalHeader()
            if header:
                # Получаем цвет фона заголовка
                header_color = header.palette().color(QPalette.Window)
                brightness = (header_color.red() + header_color.green() + header_color.blue()) / 3
                # Если яркость низкая - темная тема
                is_dark = brightness < 128
        except:
            # Fallback: проверяем stylesheet приложения
            stylesheet = app.styleSheet()
            if stylesheet:
                # Ищем признаки темной темы в stylesheet
                if '#1e1e1e' in stylesheet or '#252525' in stylesheet or '#262626' in stylesheet:
                    is_dark = True
                elif '#ffffff' in stylesheet or '#f8f9fa' in stylesheet:
                    is_dark = False
            else:
                # Если не определили, проверяем палитру
                palette = app.palette()
    ...
    def update_corner_button_theme(self):
        """Публичный метод для обновления стилей corner button при смене темы"""
        if hasattr(self, '_corner_button') and self._corner_button:
    ...
    def _select_all_table(self):
        ...
        # Выделяем все ячейки от (0,0) до последней
        last_row = self.rowCount() - 1
        last_col = self.columnCount() - 1
        if last_row >= 0 and last_col >= 0:
            # Используем selectionModel для выделения всего диапазона
